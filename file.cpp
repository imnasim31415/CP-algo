...
//    fast;
    ...
    while(cin>>n>>a>>b)
    {
        pair<pair<LL,LL>,LL>pp[n+1];
        pp[0].fi.fi=pp[0].fi.se=pp[0].se= 0;
        Rep(i,n)
        {
            cin>>pp[i].fi.fi>>pp[i].fi.se;
            pp[i].se=i;
        }
        sort(pp,pp+n+1);
        ...
        for(i=1;i<=n;i++)
        {
            if(pp[i].fi.fi<=a&&pp[i].fi.se<=b)cnt;
            ...
            for(int j=1;j<i;j++)
            {
                if(pp[j].fi.fi<pp[i].fi.fi&&pp[j].fi.se<pp[i].fi.se)
                {
                    if(dp[j]+1>dp[i])
                    {
                        dp[i]=dp[j]+1;
